...
public class TGTest {
    @Test
    public void DocumentImplTest() throws URISyntaxException {
    ...
    public void StackImplTest() {
        ...
        stack.push(123);
        assertEquals((Integer)123, stack.peek());
        assertEquals((Integer)123, stack.pop());
    }

    @Test
    public void HashTableImplTest() {
        BTree<String, Integer> table = new BTreeImpl<>();
        table.put("WHY", 12);
        table.put("Do", 13);
        table.put("I Do", 14);
        table.put("Silly Things", 15);
        table.put("?", 16);
        assertEquals((Integer)12, table.get("WHY"));
        assertEquals((Integer)16, table.get("?"));
        assertNull(table.get("ALWAYS"));
        table.put("Always", 73);
        assertEquals((Integer)13, table.put("Do", 28347));
        assertEquals((Integer)28347, table.get("Do"));
        assertEquals((Integer)73, table.get("Always"));
        BTree<Integer, Integer> test2 = new BTreeImpl<>();
        for (int i = 0; i < 4000; i++) {
            test2.put(i, i + 1);
        }
        assertEquals((Integer)4000, test2.get(3999));
        assertEquals((Integer)2348, test2.put(2347, 1));
        assertEquals((Integer)1, test2.get(2347));
    }

    @Test
    public void DocStoreTest() throws URISyntaxException, IOException {
        ...
        for (int i = 0; i < 30; i++) {
            ...
            assertEquals(0, store.putDocument(new ByteArrayInputStream(str.getBytes()), uri34,
        ...
        // Change the Value
        assertEquals(doc225.hashCode(),
                store.putDocument(new ByteArrayInputStream("1".getBytes()), x, DocumentStore.DocumentFormat.TXT));
        // Delete
