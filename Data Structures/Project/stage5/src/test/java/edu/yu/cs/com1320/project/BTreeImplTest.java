...
public class BTreeImplTest {
    ...
    public void hashTableImplSimplePutAndGet() {
        BTree<Integer, Integer> hashTable = new BTreeImpl<Integer, Integer>();
        hashTable.put(1, 2);
        hashTable.put(3, 6);
        hashTable.put(7, 14);
    ...
    public void hashTableDelNullPut() {
        BTree<String, Integer> hashTable = new BTreeImpl<String, Integer>();
    ...
    public void basicCollision() {
        BTree<Integer, String> hashTable = new BTreeImpl<Integer, String>();
        hashTable.put(1, "Avi");
    ...
    public void HashEqualButNotEqual() {
        BTreeImpl<String, Integer> hashTable = new BTreeImpl<String, Integer>();
    ...
    public void movetoDiskTest() throws Exception {
        BTree<URI, Document> bt = new BTreeImpl<>();
        bt.setPersistenceManager(new DocumentPersistenceManager(null));
        URI uri1 = new URI("this/web/site");
        Document doc1 = new DocumentImpl(uri1, "this is the text of the document 1");
        bt.put(uri1, doc1);
        bt.moveToDisk(uri1);
        assertEquals(doc1, bt.get(uri1));
    }
    
    @Test
    public void movetoDiskAndPutTest() throws Exception {
        BTree<URI, Document> bt = new BTreeImpl<>();
        bt.setPersistenceManager(new DocumentPersistenceManager(null));
        URI uri1 = new URI("this/web/site");
        Document doc1 = new DocumentImpl(uri1, "this is the text of the document 1");
        ...
        bt.put(uri1, doc1);
        bt.moveToDisk(uri1);
        assertEquals(doc1, bt.put(uri1, new DocumentImpl(uri1, "txt")));

        URI uri2 = new URI("this/web/page");
        byte[] b1 = {22, 31, 116, 52, 9};
        Document doc2 = new DocumentImpl(uri1, b1);
        bt.put(uri2, doc2);
        bt.moveToDisk(uri2);
        byte[] returnBytes = bt.get(uri2).getDocumentBinaryData();
        assertEquals(5, returnBytes.length);
        for (int i = 0; i < returnBytes.length; i++) {
            assertEquals(b1[i], returnBytes[i]);
        }
        
        
        URI uri3 = new URI("yaeyiya/");
        bt.put(uri3, new DocumentImpl(uri3, "txt"));
        bt.moveToDisk(uri3);
        assertEquals("txt", bt.get(uri3).getDocumentTxt());
    }
}