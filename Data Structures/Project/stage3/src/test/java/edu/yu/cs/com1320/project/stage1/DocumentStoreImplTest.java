...
public class DocumentStoreImplTest {
    ...
    @Test
    public void TestDocumentStoreBytes() throws Exception {
        DocumentStore dStore = new DocumentStoreImpl();
        byte[] b = new byte[20];
        new Random().nextBytes(b);
        int a = dStore.putDocument(new ByteArrayInputStream(b), new URI("test1"), DocumentFormat.BINARY);
        Document d = dStore.getDocument(new URI("test1"));
        assertArrayEquals(b, d.getDocumentBinaryData());
        assertEquals(null, d.getDocumentTxt());
        assertEquals(0, a);
    }
    ...
    public void testDelete() {
        ...
        for (int i = 0; i < 21; i++) {
            try {
                documentStore.putDocument(new ByteArrayInputStream(stringArray[i].getBytes()), uriArray[i],
                        DocumentStore.DocumentFormat.BINARY);
            } catch (java.io.IOException e) {
                fail();
            }
